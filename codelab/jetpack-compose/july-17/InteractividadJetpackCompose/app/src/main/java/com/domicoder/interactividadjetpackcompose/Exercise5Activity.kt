...
class Exercise5Activity : ComponentActivity() {
        ...
        setContent {
            ...
            InteractividadJetpackComposeTheme {
                Scaffold(modifier = Modifier.fillMaxSize()) { innerPadding ->
                    ...
                    Column {
                        Text(
                            text = "Solo poner esta linea en el ejercicio 1: var contador by rememberSaveable { mutableIntStateOf(0) }",
                            modifier = Modifier.padding(innerPadding)
                        )
                        GeneralButton(text = "< Regresar al inicio", color = Green, onClick = { Util.showMainActivity(context) }, modifier = Modifier.padding(innerPadding))
                    }
                }
            }
        }
